...
bool saveJPEGGrayscale(const char *filename, const BMPImage* img)
{
    ...
    // Huffman Kodovanje (generisanje finalnog niza bajtova)
    JpegEncoderBuffer *buffer = encodeHuffman(rld, zzd->totalBlocks);

    if (!buffer) {
        printf("Error: Compression failed at Huffman stage.\n");
        fclose(file);
        return false;
    }
    ...
    // --- 2. PISANJE ZAGLAVLJA (HEADERS) ---
    
    bool ok = true;

    // SOI (Start of Image) je deo APP0 funkcije kod tebe (0xFFD8)
    ok &= write_app0(file);
    
    // DQT (Quantization Table)
    ok &= write_dqt(file);
    
    // SOF0 (Start of Frame - dimenzije)
    ok &= write_sof0(file, yImage->width, yImage->height);
    
    // DHT (Huffman Tables) - Moramo upisati i DC i AC tabelu
    ok &= write_dht_dc(file);
    ok &= write_dht_ac(file);
    
    // SOS (Start of Scan) - Najavljuje početak podataka
    ok &= write_sos(file);
    ...
    // --- 3. PISANJE KOMPRESOVANIH PODATAKA (BITSTREAM) ---
    
    // Ovo je deo koji je falio!
    // buffer->data sadrži sirove bajtove koje je Huffman generisao.
    size_t written = fwrite(buffer->data, 1, buffer->size, file);
    
    if (written != buffer->size) {
        printf("Error writing bitstream data.\n");
        ok = false;
    } else {
    ...
    // --- 5. ČIŠĆENJE MEMORIJE (CLEANUP) ---
    // Veoma važno osloboditi sve redom, unazad
